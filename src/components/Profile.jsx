...
const Profile = () => {
  ...
  const updateUser = async () => {
    try {
      ...
      if (formData.avatar && formData.avatar instanceof File) {
        ...
        avatarFormData.append('avatar', formData.avatar) // Changed from 'file' to 'avatar'
        
        // Removed userId from query params as it's now handled by the backend
        const response2 = await fetch(`${API_BASE_URL}/api/users/upload`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
        ...
        setUser({...updatedUser, avatar: uploadData.data?.user?.avatar})
        
        // Update localStorage with new user data
        localStorage.setItem('user', JSON.stringify({...updatedUser, avatar: uploadData.data?.user?.avatar}))
      } else {
        // No file upload, just update with user info
        setUser(updatedUser)
  ...
  const getLiked = async () => {
    try {
      ...
      // Fixed array access - should be data directly, not data[0]
  ...
  return (
    <div className="mt-8 sm:mt-12 lg:mt-[55px] mx-4 sm:mx-8 lg:ml-[60px] flex flex-col p-4 pb-10 sm:pb-8 lg:pb-4">
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 p-4 lg:p-[40px]">
        ...
        {/* Dropdown Menu */}
        { isOpen && 
          <div className="absolute top-full right-4 sm:right-2 lg:static lg:top-auto lg:right-auto 
            flex flex-col items-start transform lg:translate-x-0
          ...
          <div className="bg-white w-full max-w-md sm:max-w-lg lg:w-[600px] h-auto max-h-[90vh] p-4 rounded-lg overflow-y-auto">
            <div className="flex flex-row justify-between items-center mb-4"> 
              ...
              <XMarkIcon className="w-6 h-6 cursor-pointer 
                hover:rounded-[50%] hover:p-1 hover:bg-gray-200" 
